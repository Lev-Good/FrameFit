...
```
┌──────────────────────────────────────────────────────────────┐
│  שכבה 1 — שטח עבודה (AppBar) + סרגל המשימות                   │
│  מצמצם את rcWork עד גבול האזור הגלוי ⇒ מקסם, Snap מכבדים      │
├──────────────────────────────────────────────────────────────┤
...
├── tests/
│   └── FrameFit.Core.Tests/            # xUnit — 65 בדיקות
└── docs/
```
...
* רושם **סרגלי יישום (AppBars)** על ארבעת שולי המסך הנבחר דרך `SHAppBarMessage`
  (`ABM_NEW`, `ABM_QUERYPOS`, `ABM_SETPOS`, `ABM_REMOVE`), עם חלון מוסתר לכל צד.
* **קיזוז הקצאות קיימות (D12):** לפני הרישום נמדד `rcWork` בפועל (`WorkAreaProbe`), ובכל צד
  מוזמן רק ה**הפרש** מול מה שמערכת ההפעלה כבר תפסה בו — סרגל המשימות בראשן. החישוב עצמו
  יושב בליבה הנבדקת (`WorkAreaCalculator.Plan`), והוא מחזיר גם "חוסר" — פיקסלים מהאזור
  הגלוי שלא ניתן להחזיר. `ABM_QUERYPOS` מספק את מיקום הצד; אנחנו קובעים רק את המידה.
* **רצועת הסרגל המעוגן (D13):** כאשר סרגל המשימות הועבר אל תוך האזור הגלוי (ברירת המחדל),
  גובהו נגרע מאזור העבודה: `Plan` מקבל `bottomAnchor` ומחזיר אזור עבודה שמסתיים בקצהו העליון
  של הסרגל (`WorkAreaPlan.BottomAnchor`). התוצאה: התוכן והסרגל מחלקים את האזור הגלוי ביניהם
  בלי חור ובלי חפיפה — אותו מצב שהבדיקה העצמית מאמתת בפועל.
* התוצאה: `rcWork` של אותו מסך **מסתיים בדיוק בגבול האזור הגלוי** — וזו המשמעות של
  "אזור העבודה" במערכת ההפעלה. אומת בבדיקה עצמית בשני מצבי שוליים (רדודים ועמוקים).
* **זהו נכס האכיפה החזק ביותר**, משום שהוא מכבד ע"י מנגנוני המערכת עצמם, ולכן חלון ממוקם
  (maximize) ו-Snap נופלים בדיוק על האזור הגלוי. דוח האבחון מציג את אזור העבודה שהתקבל מול
  זה המצופה, ואת ההקצאה שנשארה תפוסה.

### 3.2.1 `TaskbarHost` — סרגל המשימות של Windows (Platform)
* **הבעיה:** סרגל המשימות מוצמד לשולי המסך, מקצה לעצמו מקום באזור העבודה ואינו נעלם מעצמו;
  הוא גם חלון on-top של ה-Shell ולכן מצויר **מעל** שכבת הכיסוי. בשני המובנים הוא מפריע.
* **מה שנמדד (D14):** אי אפשר להזיז את סרגל המשימות. ה-Shell של Windows 11 מחזיק אותו
  בקצה המסך ומחזיר אותו לשם מיד לאחר כל הזזה — ב-300 דגימות רצופות (25 הרץ, 12 שניות)
  הסרגל לא שהה ביעד אף לא פעם אחת, ו-`SetWindowPos` בכל זאת מחזיר הצלחה. הכיסוי של
  `MoveInto` נשאר בקוד כניסוי, אך הוא **מאמת את המיקום שקוראים חזרה מהמערכת**
  (`ReadCurrentRect`, חלון אימות של 350 מ״ש) ומדווח על כישלון במקום להסתמך על הצלחת הקריאה.
* **ברירת המחדל (D14):** אי-התערבות. הסרגל נשאר במקומו, וקיזוז ההקצאה שלו (FR-5.1) מבטיח
  שלא נגזל אף פיקסל גלוי. שמירה קצרה (כל שנייה) פעילה רק כשמשהו אכן הועבר.
* **המסלולים החלופיים:** `ShowWindow(SW_HIDE)` (הסתרה מלאה — מספקת שוליים שחורים ואחידים),
  או ניסיון העברה עם אימות. בכל המצבים ההחזרה היא בכל מסלול יציאה.
* **מה שלא נוגעים בו:** כשהסרגל *מוסתר* ע"י מישהו אחר (ה-Shell מסתיר אותו לבדו כשחלון נכנס
  למסך מלא) — לא מחזירים אותו ולא מזיזים אותו בכוח, כדי שסרגל on-top לא יופיע מעל יישום
  במסך מלא.
* **רשת ביטחון:** קובץ סימון ב-`%APPDATA%\FrameFit\taskbar-hidden.marker` נכתב בזמן ההסתרה
  ונמחק בהחזרה. אם התהליך נהרג — בעלייה הבאה הסימון מחזיר את הסרגל למקומו הטבעי לפני כל
  פעולה אחרת.
* **מה שנמדד ואינו מובן מאליו:** הסתרת הסרגל **אינה** משחררת את ההקצאה שלו באזור העבודה
  (`rcWork` נשאר 1920×1032 במסך 1920×1080). לכן הקיזוז שבסעיף 3.2 הוא תנאי הכרחי, וסרגל
  המשימות נמדד ומודפס כהקצאה קיימת גם כשהוא מוסתר.
...
| `SHAppBarMessage` | הזמנת שוליים ⇒ הקטנת `rcWork` | מקוזז מול הקצאות קיימות — ראו D12 |
| `EnumWindows`, `GetClassName`, `ShowWindow` | איתור, הסתרה והחזרה של סרגל המשימות | ראו D12 |
...
| ~~אזור העבודה קצר מהאזור הגלוי בגובה סרגל המשימות~~ | רצועה מתה וגזילת פיקסלים גלויים | **הוסר בדיווח מהשטח: קיזוז ההקצאות הקיימות (3.2) + הסתרת סרגל המשימות (3.2.1). ראו D12.** |
| הסתרת סרגל המשימות ואי-החזרתו (למשל אחרי הריגת התהליך) | המשתמש נותר בלי סרגל | קובץ סימון שמחזיר אותו בעלייה הבאה; מקש מילוט; החזרה בכל מסלול יציאה |
